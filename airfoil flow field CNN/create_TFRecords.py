...
image_path = "./6400_crop"
pressure_path = "./pressure_data_half.csv"
IMG_HEIGHT = 252
IMG_WIDTH = 400
IMG_CHANNEL = 3
...
def read_images_and_pressure(image_path, pressure_path):
    ...
    "这里图片的数据类型需要是无符号整型"
    images = np.zeros(shape=(num_files, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNEL),
                      dtype=np.uint8)
    features = np.zeros(shape=(17497, 10), dtype=np.float32)
    ...
    for i, image in enumerate(new_file_list):
        ...
        "将图片的尺寸缩小了一半"
        "好像是这里出现了问题，resize的维度不正确"
        try:
            img = cv2.resize(img, (IMG_WIDTH, IMG_HEIGHT))
            # resize img.shape (252, 400, 3)
            print("resize img.shape", img.shape)
            images[i] = img
            # images[17496].shape (252, 400, 3), <class 'numpy.ndarray'>
            print("images[%d].shape %s, %s" % (i,images[i].shape, type(images[i])))
        except:
            print("Error: could not resize")

    print("images.shape", images.shape)
    # images.shape (17497, 252, 400, 3)
    print("features.shape", features.shape)
    # features.shape (17497, 10)
