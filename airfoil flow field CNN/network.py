...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('input_reshape') as scope:
        ...
        h0 = tf.nn.relu(bn_0(h0))
        print("h0.shape", h0.get_shape().as_list())

        "接下来进行转置卷积"

        # h1 = deconv2d(input_=h0, output_shape=[batch_size, s_h8, s_w8, f_dim*4], name="h1_deconv")
        h1 = tf.layers.conv2d_transpose(inputs=h0, filters=f_dim*4, kernel_size=5, strides=(2,2), padding="SAME")
        "tf.layers.conv2d_transpose之后不可以加入reshape,会报错"
        # 转置卷积之后--> [32, 32, 50, 256]
        # h1 = tf.reshape(h1, [batch_size, s_h8, s_w8, f_dim*4])
        h1 = tf.nn.relu(bn_1(h1))
        # h1.shape [32, 32, 50, 256]
        print("h1.shape", h1.get_shape().as_list())

        # h2 = deconv2d(input_=h1, output_shape=[batch_size, s_h4, s_w4, f_dim*2], name="h2_deconv")
        h2 = tf.layers.conv2d_transpose(inputs=h1, filters=f_dim*2, kernel_size=5, strides=(2,2), padding="SAME")
        # 转置卷积之后--> [32, 63, 100, 128]
        # h2 = tf.reshape(h2, [batch_size, s_h4, s_w4, f_dim*2])
        h2 = tf.nn.relu(bn_2(h2))
        # h2.shape [None, 64, 100, 128]
        print("h2.shape", h2.get_shape().as_list())

        # h3 = deconv2d(input_=h2, output_shape=[batch_size, s_h2, s_w2, f_dim], name="h3_deconv")
        h3 = tf.layers.conv2d_transpose(inputs=h2, filters=f_dim*1, kernel_size=5, strides=(2,2), padding="SAME")
        # h3.shape [None, 128, 200, 64]
        h3 = tf.nn.relu(bn_3(h3))
        print("h3.shape", h3.get_shape().as_list())

        # h4 = deconv2d(input_=h3, output_shape=[batch_size, s_h, s_w, channel_dim], name="h4_deconv")
        h4 = tf.layers.conv2d_transpose(inputs=h3, filters=channel_dim, kernel_size=5, strides=(2,2), padding="SAME")
    ...
    with tf.variable_scope("conv1") as scope:
        ...
        with tf.name_scope("weights"):
            conv1_weights = tf.get_variable(name="conv1_weights", shape=[CONV1_SIZE, CONV1_SIZE, IMG_CHANNEL, CONV1_DEPTH],
                                            initializer=tf.truncated_normal_initializer(stddev=0.1), )
            variable_summaries(conv1_weights, "conv1" + "/weights")
        with tf.name_scope("biases"):
            conv1_biases = tf.get_variable(name="conv1_bias", shape=[CONV1_DEPTH], initializer=tf.constant_initializer(0.0),)
            variable_summaries(conv1_biases, "conv1" + "/biases")
        # 卷积步长为2，"SAME" ，图像尺寸减半
        conv1 = tf.nn.conv2d(h4, conv1_weights, strides=[1, 2, 2, 1], padding="SAME")
    ...
    with tf.variable_scope("conv2") as scope:
        ...
        with tf.name_scope("weights"):
            conv2_weights = tf.get_variable(name="conv2_weights", shape=[CONV2_SIZE, CONV2_SIZE, CONV1_DEPTH, CONV2_DEPTH],
                                            initializer=tf.truncated_normal_initializer(stddev=0.1), )
            variable_summaries(conv2_weights, "conv2" + "/weights")
        with tf.name_scope("biases"):
            conv2_biases = tf.get_variable("conv2_bias", shape=[CONV2_DEPTH], initializer=tf.constant_initializer(0.0),
                                           )
    ...
    with tf.variable_scope("fullly_reshape") as scope:
        ...
        # [batch_size, 63*100*64]
        reshaped = tf.reshape(pool2, [-1, nodes])
        # reshaped shape [8, 403200]
        print("reshaped shape", reshaped.get_shape().as_list())

    with tf.variable_scope("fully1") as scope:

        with tf.name_scope("weights"):
            fc1_weights = tf.get_variable(name="fc1_weights", shape=[nodes, FC_NODE],
                                            initializer=tf.truncated_normal_initializer(stddev=0.1))
        ...
        with tf.name_scope("biases"):
            fc1_biases = tf.get_variable(name="fc1_biases", shape=[FC_NODE], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope("fully2") as scope:
        ...
        with tf.name_scope("weights"):
            fc2_weights = tf.get_variable(name="fc2_weights", shape=[FC_NODE, NUM_LABELS],
                                          initializer=tf.truncated_normal_initializer(stddev=0.1))
        ...
        with tf.name_scope("biases"):
            fc2_biases = tf.get_variable(name="fc2_biases", shape=[NUM_LABELS], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope("output_reshape") as scope:
        ...
        # output_shape [64, 252, 400, 3]
        # output_shape.dtype <dtype: 'float32'>
        # output_shape [None, 252, 400, 3]
        print("output_shape.dtype", output_array.dtype)
